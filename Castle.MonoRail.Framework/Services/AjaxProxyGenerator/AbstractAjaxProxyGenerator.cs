namespace Castle.MonoRail.Framework.Services.AjaxProxyGenerator
{
    ...
    public abstract class AbstractAjaxProxyGenerator : IMRServiceEnabled, IAjaxProxyGenerator
    {
        ...
        private bool wrapProxyInScriptBlock = true;
        ...
        /// <summary>
        /// Deterains if the proxy generated by this instance will
        /// be wrapped in a &lt;script&gt; block or not.
        /// </summary>
        public bool WrapProxyInScriptBlock
        {
            get { return wrapProxyInScriptBlock; }
            set { wrapProxyInScriptBlock = value;}
        }
        ...
        private static ScriptFunctionParameter[] GetScriptFunctionParameters(IEnumerable<ParameterInfo> parameters)
        {
            ...
            {
                var clientSideName = GetParameterName(parameterInfo);
                var serverSideName = clientSideName;

                var needsJSONEncoding = false;
        ...
        {
            var attributes = obj.GetCustomAttributes(attributeType, inherit);
        ...
        {
            var baseUrl = context.ApplicationPath + "/";
        ...
        private static object GetPropertyValue(object obj, string propName)
        {
            ...
            var propertyInfo = obj.GetType().GetProperty(propName, BindingFlags.Public | BindingFlags.Instance);
        ...
        private static string GetParameterName(ParameterInfo paramInfo)
        {
            ...
            if (ARFetchAttType != null)
            {
                ...
                var parameterAttr = GetSingleAttribute(paramInfo, ARFetchAttType, true);
