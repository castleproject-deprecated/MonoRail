...
module ResourceMetadataBuilder =
    ...
    let PropertiesBindingFlags = BindingFlags.Public ||| BindingFlags.Instance ||| BindingFlags.FlattenHierarchy

    let is_primitive (pType:Type) = 
        (ResourceType.GetPrimitiveResourceType pType) <> null

    let resolve_name (pType:Type) (type2CustomName:Dictionary<Type, string>) = 
        let res, name = type2CustomName.TryGetValue(pType)
        if res then name
        else pType.Name

    let resolve_resourceTypeKind_based_on_properties (entType:Type) = 
    ...
    let extract_elementType (pType:Type) = 
    ...
    let rec resolveRT (pType) (knownTypes:Dictionary<Type, ResourceType>) (builderFn) = 
    ...
    let resolve_propertKind (resource:ResourceType) (propertyInfo:PropertyInfo) (isCollection) = 
    ...
    let build_property (prop:PropertyInfo) (knownTypes:Dictionary<Type, ResourceType>) builderFn  = 
        ...
        if not prop.CanRead || prop.GetIndexParameters().Length <> 0 then
            ...
            | Some (resolvedType, isColl) ->
                ...
                Some(ResourceProperty(prop.Name, kind, resolvedType))
            | _ -> None


    let build_properties (resource:ResourceType) (knownTypes:Dictionary<Type, ResourceType>) (type2CustomName:Dictionary<Type, string>) resourceBuilderFn (entType:Type)  = 
    ...
    let rec build_resource_type schemaNs (knownTypes:Dictionary<Type, ResourceType>) (type2CustomName:Dictionary<Type, string>) (entType:Type) = 
        ...
        if entType.IsValueType || not entType.IsVisible || entType.IsArray || entType.IsPointer || entType.IsCOMObject || entType.IsInterface || 
           ...
           null
            ...
            build_properties resource knownTypes type2CustomName (build_resource_type schemaNs knownTypes type2CustomName) entType
            resource

    /// Asserts that the return from build_resource_type is non null and EntityType
    let build_entity_resource schemaNs (config:EntitySetConfig) (knownTypes) (type2CustomName) = 
        let resource = build_resource_type schemaNs knownTypes type2CustomName config.TargetType
    ...
    let public build(schemaNs:string, configs:EntitySetConfig seq) = 
