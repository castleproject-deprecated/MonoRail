...
namespace Castle.MonoRail
    ...
    type ODataModel(schemaNamespace, containerName) = 
        class
            ...
            let _rt2ControllerCreator : Ref<Dictionary<ResourceType, Func<ControllerPrototype>>> = ref null
            ...
            member internal x.GetNestedOperation (rt:ResourceType, name:string) = 
                if !_rt2ControllerCreator <> null then
                    let succ, cp = (!_rt2ControllerCreator).TryGetValue rt
                    if succ then
                        cp.H
            ...
            member internal x.SetServiceRegistry(services:IServiceRegistry) = 
                if _serviceRegistry.Value = null then 
                    ...
                    let resolve_creator (entityType:Type) = 
                    ...
                    let build_dict = 
                        let dict = Dictionary<ResourceType, Func<ControllerPrototype>>()
                        _resourcetypes.Force() 
                        |> Seq.map (fun rt -> rt, resolve_creator rt.InstanceType)
                        |> Seq.filter (fun t -> snd t <> null)
                        |> Seq.iter (fun t -> dict.Add (fst t, snd t))
                        dict
                    
                    _rt2ControllerCreator := build_dict    
